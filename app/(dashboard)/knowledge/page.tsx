  ...
  return (
    <div className="p-8 pb-20 max-w-[800px] animate-in fade-in duration-300">
      <div className="mb-8">
        ...
        <p className="text-sm text-gray-500 mt-1 leading-relaxed">Add data sources for your bot to learn from. Upload text docs, crawl sitemaps, or sync GitBook spaces.</p>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-md p-6 mb-8 shadow-sm">
        <div className="flex space-x-2 bg-gray-50 p-1 rounded-lg border border-gray-200 mb-6 max-w-sm">
          {(['website', 'gitbook', 'file'] as const).map(tab => (
            <button key={tab} onClick={() => setSourceType(tab)} className={`flex-1 py-1.5 text-xs font-semibold rounded-md transition-all ${sourceType === tab ? 'bg-white shadow-sm text-gray-900 border border-gray-200/50' : 'text-gray-500 hover:text-gray-700'}`}>
            ...
            {sources.map(src => (
              <div key={src.id} className="p-6 flex justify-between items-center hover:bg-gray-50/50 transition-colors">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1 font-medium">{src.source_uri}</p>
                </div>
