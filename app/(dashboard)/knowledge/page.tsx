...
export default function KnowledgeBasePage() {
  ...
  if (isLoading) return <div className="flex h-full items-center justify-center bg-[#FAFAFA]" />;
  ...
  return (
    <div className="p-8 max-w-[1000px] mx-auto overflow-y-auto h-full bg-[#FAFAFA]">
      <div className="mb-8">
        ...
        <p className="text-gray-500 text-sm">Add data sources for your bot to learn from.</p>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-sm p-6 mb-8 shadow-sm">
        ...
        {sourceType === 'website' && (
          <div className="flex gap-3 max-w-md">
            <input type="url" placeholder="https://example.com/sitemap.xml" className="flex-1 p-2.5 border border-gray-200 rounded-md text-sm outline-none focus:border-black" value={inputValue} onChange={(e) => setInputValue(e.target.value)} />
            <button onClick={() => handleAddSource({ type: 'website', url: inputValue })} disabled={isSyncing || !inputValue} className="px-5 bg-black text-white text-sm font-medium rounded-md hover:bg-gray-800 disabled:opacity-50">
        ...
        {sourceType === 'gitbook' && (
          <div className="flex flex-col gap-3 max-w-md">
            <input type="text" placeholder="GitBook Space ID" className="w-full p-2.5 border border-gray-200 rounded-md text-sm outline-none focus:border-black" value={inputValue} onChange={(e) => setInputValue(e.target.value)} />
            <input type="password" placeholder="GitBook API Token" className="w-full p-2.5 border border-gray-200 rounded-md text-sm outline-none focus:border-black" value={gitbookToken} onChange={(e) => setGitbookToken(e.target.value)} />
            <button onClick={() => handleAddSource({ type: 'gitbook', gitbookSpaceId: inputValue, apiKey: gitbookToken })} disabled={isSyncing || !inputValue || !gitbookToken} className="self-end px-5 py-2.5 bg-black text-white text-sm font-medium rounded-md hover:bg-gray-800 disabled:opacity-50">
        ...
        {sourceType === 'file' && (
          <div>
            ...
            <button onClick={() => fileInputRef.current?.click()} disabled={isSyncing} className="px-5 py-2.5 bg-white border border-gray-200 text-gray-700 text-sm font-medium rounded-md hover:bg-gray-50 transition-colors shadow-sm">
      ...
      <div className="bg-white border border-gray-200 rounded-sm">
        <div className="p-5 border-b border-gray-100"><h2 className="text-sm font-semibold text-gray-900">Active Sources</h2></div>
        {sources.length === 0 ? (
          <div className="p-8 text-center text-sm text-gray-500">No data sources added yet.</div>
        ) : (
          <div className="divide-y divide-gray-100">
            {sources.map(src => (
              <div key={src.id} className="p-5 flex justify-between items-center hover:bg-gray-50/50">
                <div>
                ...
                <button onClick={() => removeSource(src.id)} className="text-sm text-red-500 hover:text-red-700 font-medium">Remove</button>
              </div>
            ))}
          </div>
        )}
