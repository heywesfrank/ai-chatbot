  ...
  return (
    <div className="flex flex-col h-full w-full bg-[#FAFAFA] text-gray-900 font-sans overflow-y-auto">
      <div className="max-w-[1200px] mx-auto w-full p-4 sm:p-8 pb-20">
        ...
        {!isOwner && (
          <div className="bg-blue-50 border border-blue-100 p-4 rounded-md text-sm text-blue-700 flex items-start sm:items-center gap-2 mb-6 font-medium">
             <svg className="w-4 h-4 mt-0.5 sm:mt-0 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
             <span>You are viewing this workspace as an Agent. Only the owner can invite or remove members.</span>
          </div>
        )}
          ...
          <div className="p-4 sm:p-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <form onSubmit={handleAddMember} className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
              <input
                ...
                disabled={!isOwner}
                className="flex-1 sm:w-64 p-2.5 border border-gray-200 rounded-sm text-sm outline-none focus:border-black transition-colors disabled:bg-gray-50 disabled:text-gray-500"
              ...
              <button
                ...
                className="px-5 py-2.5 sm:py-0 bg-black text-white text-sm font-medium rounded-sm hover:bg-gray-800 disabled:opacity-50 transition-colors"
              >
                Invite
              </button>
            </form>
          ...
          <div className="divide-y divide-gray-100">
            {teamMembers.length > 0 ? (
              teamMembers.map((member) => (
                <div key={member.id} className="flex items-center justify-between p-4 sm:p-6 hover:bg-gray-50/50 transition-colors">
                  <div className="flex items-center gap-3 sm:gap-4 min-w-0 pr-4">
                    <div className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center border border-gray-200 text-gray-500 font-medium shrink-0">
                  ...
                  <button
                    ...
                    disabled={!isOwner}
                    className="text-sm text-red-500 hover:text-red-700 font-medium disabled:opacity-30 disabled:cursor-not-allowed transition-colors shrink-0"
                  >
                    Remove
                  </button>
                </div>
