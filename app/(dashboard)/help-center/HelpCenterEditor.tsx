...
export default function HelpCenterEditor({ article, activeSpaceId, allCategories, allArticles, onClose, onSuccess }: HelpCenterEditorProps) {
  ...
  return (
    <div className="flex flex-col h-full w-full bg-[#FAFAFA] text-gray-900 font-sans">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between px-4 sm:px-8 py-4 sm:py-5 border-b border-gray-200 shrink-0 bg-white z-10 gap-4 lg:gap-0">
        <div className="flex items-center gap-4">
          <button onClick={onClose} className="flex items-center gap-2 text-sm font-medium text-gray-500 hover:text-gray-900 transition-colors shrink-0">
            <ArrowLeftIcon className="w-4 h-4" />
            Back
          </button>
          <div className="h-4 w-px bg-gray-200 shrink-0" />
          <span className="text-sm font-medium text-gray-900 line-clamp-1">{currentId ? 'Edit Article' : 'New Article'}</span>
          <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide shrink-0 ${status === 'published' ? 'bg-green-100 text-green-700' : status === 'archived' ? 'bg-gray-200 text-gray-700' : 'bg-amber-100 text-amber-700'}`}>
            {status}
          </span>
        </div>
        
        <div className="flex items-center gap-3 overflow-x-auto pb-1 lg:pb-0 hide-scrollbar">
          <div className="bg-gray-100 p-0.5 rounded-md flex items-center mr-0 sm:mr-2 shrink-0">
            <button onClick={() => setIsPreview(false)} className={`px-3 py-1.5 text-xs font-medium rounded-sm transition-all ${!isPreview ? 'bg-white text-black shadow-sm' : 'text-gray-500 hover:text-gray-900'}`}>Edit</button>
            <button onClick={() => setIsPreview(true)} className={`px-3 py-1.5 text-xs font-medium rounded-sm transition-all ${isPreview ? 'bg-white text-black shadow-sm' : 'text-gray-500 hover:text-gray-900'}`}>Preview</button>
          </div>
          {lastSaved && <span className="text-[10px] text-gray-400 mr-2 font-medium hidden sm:block shrink-0">Saved {lastSaved.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>}
          {currentId && status === 'published' && (!scheduledAt || new Date(scheduledAt) <= new Date()) && visibility === 'public' && (
            <a href={`/help/${activeSpaceId}/${slug || currentId}`} target="_blank" className="text-xs font-medium text-gray-500 hover:text-gray-900 transition-colors items-center gap-1.5 px-3 py-2 rounded-md hover:bg-gray-50 border border-transparent hidden sm:flex shrink-0">
              View Live <ExternalLinkIcon className="w-3.5 h-3.5" />
            </a>
          )}
          <button onClick={() => handleSave('archived')} disabled={isSaving || !title.trim() || !content.trim()} className="px-4 py-2.5 bg-white border border-gray-200 text-red-600 text-xs font-medium rounded-md hover:bg-red-50 transition-colors disabled:opacity-50 hidden sm:block shrink-0">
            Archive
          </button>
          <button onClick={() => handleSave('draft')} disabled={isSaving || !title.trim() || !content.trim()} className="px-4 py-2.5 bg-white border border-gray-200 text-gray-700 text-xs font-medium rounded-md hover:bg-gray-50 transition-colors disabled:opacity-50 shrink-0">
            Save Draft
          </button>
          <button onClick={() => handleSave('published')} disabled={isSaving || !title.trim() || !content.trim()} className="px-5 py-2.5 bg-black text-white text-xs font-medium rounded-md hover:bg-gray-800 transition-colors disabled:opacity-50 shrink-0">
            {isSaving ? 'Saving...' : (scheduledAt && new Date(scheduledAt) > new Date() ? 'Schedule' : 'Publish')}
          </button>
        </div>
      </div>

      <div className="flex flex-col lg:flex-row flex-1 overflow-y-auto lg:overflow-hidden">
        <div className="flex-1 lg:overflow-y-auto p-4 sm:p-8 bg-white">
          <div className="max-w-3xl mx-auto w-full flex flex-col gap-6">
            <div className="flex items-center justify-between gap-4">
              <input type="text" placeholder="Article Title" className="flex-1 text-2xl sm:text-4xl font-bold text-gray-900 placeholder:text-gray-300 outline-none border-none bg-transparent min-w-0" value={title} onChange={handleTitleChange} />
              ...
              {!isPreview && (
                <div className="relative shrink-0">
                  <button onClick={() => setAiMenuOpen(!aiMenuOpen)} disabled={isGeneratingAI} className="flex items-center gap-1.5 px-3 py-2 bg-indigo-50 text-indigo-600 border border-indigo-100 rounded-md text-xs font-medium hover:bg-indigo-100 transition-colors disabled:opacity-50 shadow-sm">
                    ...
                    {isGeneratingAI ? 'AI Working...' : <span className="hidden sm:inline">AI Assistant</span>}
                  </button>
            ...
            {isPreview ? (
              ...
              <>
                <style dangerouslySetInnerHTML={{__html: `.ql-toolbar.ql-snow { border: 1px solid #E5E7EB; border-radius: 0.375rem 0.375rem 0 0; background: #FAFAFA; border-bottom: none; } .ql-container.ql-snow { border: 1px solid #E5E7EB; border-radius: 0 0 0.375rem 0.375rem; font-family: inherit; font-size: 1rem; } .ql-editor { min-height: 400px; padding: 1.5rem; color: #374151; line-height: 1.7; } .ql-editor h1, .ql-editor h2, .ql-editor h3 { color: #111827; font-weight: 600; margin-top: 1.5em; margin-bottom: 0.5em; } .ql-editor p { margin-bottom: 1em; } .ql-editor img { border-radius: 0.5rem; border: 1px solid #F3F4F6; max-width: 100%; height: auto; } .ql-syntax { background-color: #f3f4f6 !important; border: 1px solid #e5e7eb; border-radius: 0.375rem; padding: 1rem; font-size: 0.875rem; }`}} />
        ...
        <div className="w-full lg:w-[340px] shrink-0 border-t lg:border-t-0 lg:border-l border-gray-200 bg-[#FAFAFA] p-4 sm:p-6 lg:overflow-y-auto flex flex-col gap-8">
          <div>
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5 mb-2">
                  {tags.map((tag, i) => (
                    <span key={i} className="inline-flex items-center gap-1 bg-gray-200 text-gray-700 px-2 py-1 rounded-sm text-xs font-medium break-all">
                      {tag} <button type="button" onClick={() => removeTag(tag)} className="hover:text-red-500 transition-colors ml-0.5 shrink-0"><ClearIcon className="w-3 h-3"/></button>
                    </span>
                  ))}
                </div>
                  ...
                  {allArticles.filter(a => a.id !== currentId).map(a => (
                    <label key={a.id} className="flex items-center gap-2.5 p-2 hover:bg-gray-50 rounded-sm cursor-pointer transition-colors group">
                      <input 
                        ...
                        className="w-3.5 h-3.5 accent-black border-gray-300 rounded-sm focus:ring-black shrink-0" 
                      />
