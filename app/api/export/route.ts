...
export async function GET(req: Request) {
  try {
    ...
    if (!config?.space_id) return new NextResponse('No config found', { status: 400 });
    ...
    if (type === 'leads') {
      const { data } = await supabase.from('leads').select('*').eq('space_id', config.space_id).order('created_at', { ascending: false });
      csvStr = "Date,Name,Email\n";
      data?.forEach(r => {
        csvStr += `"${new Date(r.created_at).toISOString()}","${r.name?.replace(/"/g, '""')}","${r.email?.replace(/"/g, '""')}"\n`;
      });
    } else if (type === 'chats') {
      const { data } = await supabase.from('chat_feedback').select('*').eq('space_id', config.space_id).order('created_at', { ascending: false });
