...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { data: configData } = await supabase
      .from('bot_config')
      .select(
        'system_prompt, faq_overrides, language, temperature, match_threshold, reasoning_effort, verbosity'
      )
      .eq('space_id', spaceId)
      .maybeSingle();
    ...
    const matchedFaq = faqs.find(
      (f: any) =>
        f.question.toLowerCase().trim() === lastMessageContent.toLowerCase().trim()
    );
    ...
    const context =
      documents && documents.length > 0
        ? documents
            .map((doc: DocumentMatch) => `[Source: ${doc.page_url}]\n${doc.content}`)
            .join('\n\n')
        : '';

    const agentPersona =
      configData?.system_prompt || 'You are a helpful, minimalist support assistant.';
...
`.trim();
    ...
    // IMPORTANT: Cast to the streaming request type so TS selects the async-iterable overload
